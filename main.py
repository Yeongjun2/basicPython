...
def consume_item(item, t_inventory):
    if check_item(item, t_inventory):
                if t_inventory[item] <=0:
                    print("현재 아이템의 수량이 부족하여 사용할 수 없습니다.")
                else:
                    t_inventory[item] -= 1
                    print(item+ "의 수량은 " +str(t_inventory[item])+"입니다.")
    else:
...
# Todolist 2
# while문을 아이템다루기 함수로 구현
def use_item(inventory):
    while True:
        print_itemMenu()
        option = int(input("메뉴 번호를 입력하세요)"))
        if option == 0:
            print("인벤토리 조작을 종료합니다.")
            break
        elif option == 1:
             # Todolist 1'
             # 함수 인자 추가 기능 구현시 호출 수정 필요
            new_item = input("아이템을 입력하세요 : ")
            amount = int(input("아이템의 수량을 입력하세요 : " ))
            ...
            print(" 현재 당신의 아이템 입니다.")
            print(inventory)
            take_item = input(" 어떤 아이템을 사용하시겠습니까? : " )
            consume_item(take_item, inventory)
        else:
...
while True:
    ...
    if option == 0:
        ...
        if check_character(temp_name, character):
            ...
            print(temp_name+  " 은(는) 존재하지 않는 캐릭터 입니다.")
    elif option == 4:
